...
if __name__ == '__main__':
    ...
    parser.add_argument('--category', type=str, default=None, help='Specify the category to process (e.g., Box, Chair). If None, process all.')
    ...
    if args.category is not None:
        original_count = len(all_ply_files)
        # 假设文件名中包含类别字符串 (不区分大小写)
        # 例如 args.category="box", 会匹配 "Box_01.ply", "storage_box.ply" 等
        all_ply_files = [
            f for f in all_ply_files 
            if args.category.lower() in f.stem.lower()
        ]
        filtered_count = len(all_ply_files)
        Log.info(f"Filter applied: '{args.category}'. Found {filtered_count} files (from total {original_count}).")
        
        if filtered_count == 0:
            Log.error(f"No files found for category '{args.category}'. Check your filenames or path.")
            exit(0)
