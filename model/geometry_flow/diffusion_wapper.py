# [ArtFormer]: This code is adapted from Flow-ALI `https://github.com/princeton-computational-imaging/Flow-ALI`
...
class _DiffusionModel(nn.Module):
    def __init__(
        ...
        # calculations for flow q(x_t | x_{t-1}) and others
        ...
        # below: log calculation clipped because the posterior variance is 0 at the beginning of the flow chain
    ...
    def predict_start_from_noise(self, x_t, t, velocity_field):
        return (
            extract(self.sqrt_recip_alphas_cumprod, t, x_t.shape) * x_t -
            extract(self.sqrt_recipm1_alphas_cumprod, t, x_t.shape) * velocity_field
        )
    ...
    @torch.no_grad()
    def ddim_sample(self, dim, batch_size, velocity_field=None, clip_denoised = True, traj=False, cond=None):
        ...
        x_T = default(velocity_field, torch.randn(batch, dim, device = device))
        ...
        for time, time_next in tqdm(time_pairs, desc = 'sampling loop time step'):
            ...
            velocity_field = torch.randn_like(x_T) if time_next > 0 else 0.
    ...
    @torch.no_grad()
    def sample(self, dim, batch_size, velocity_field=None, clip_denoised = True, traj=False, cond=None):
        ...
        for t in tqdm(reversed(range(0, self.num_timesteps)), desc='sampling loop time step', total=self.num_timesteps):
            ...
            velocity_field = torch.randn_like(x_T) if t > 0 else 0. # no velocity_field if t == 0

            x_T = model_mean + (0.5 * model_log_variance).exp() * velocity_field
    ...
    # "nice property": return x_t given x_0, velocity_field, and timestep
    def q_sample(self, x_start, t, velocity_field=None):

        velocity_field = default(velocity_field, lambda: torch.randn_like(x_start))
        #velocity_field = torch.clamp(velocity_field, min=-6.0, max=6.0)

        return (
            extract(self.sqrt_alphas_cumprod, t, x_start.shape) * x_start +
            extract(self.sqrt_one_minus_alphas_cumprod, t, x_start.shape) * velocity_field
        )

    # main function for calculating loss
    def forward(self, x_start, t, ret_pred_x=False, velocity_field=None, cond=None):
        '''
        x_start: [B, D]
        t: [B]
        '''
        ...
        x = self.q_sample(x_start=x_start, t=t, velocity_field=velocity_field)

        model_in = (x, cond) if cond is not None else x
        model_out = self.model(model_in, t)

        if self.objective == 'pred_noise':
            target = velocity_field
