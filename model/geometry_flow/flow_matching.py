...
# Non-invasive integration with existing Flow framework
...
class FlowMatchingScheduler:
    ...
    def get_velocity_target(
        ...
        Args:
            x_0: source distribution (velocity_field)
            x_1: target distribution (clean data)
            t: time steps [0, 1]
            velocity_field: optional velocity_field for conditional flow
        ...
        if self.flow_type == 'optimal_transport':
            ...
            if velocity_field is None:
                velocity_field = torch.randn_like(x_0)
            
            x_t = mu_t + sigma_t * velocity_field
            v_t = x_1 - (1 - self.sigma_min) * velocity_field
...
class FlowMatchingLoss(nn.Module):
    ...
    def forward(
        ...
        if self.weighting == 'velocity' and t is not None:
            # Weight by time (similar to P2 weighting in flow)
...
class FlowMatchingWrapper(nn.Module):
    """
    Wrapper that adds Flow Matching capability to existing flow model
    ...
    def compute_loss(
        ...
        Args:
            ...
            velocity_field: optional velocity_field (x_0 in flow matching)
        ...
        # Generate velocity_field (source distribution)
        if velocity_field is None:
            velocity_field = torch.randn_like(x_start)
        
        # Get flow matching targets
        x_t, v_t = self.scheduler.get_velocity_target(velocity_field, x_start, t)
