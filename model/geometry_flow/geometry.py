...
class DualLossDiffusionModel(_DiffusionModel):
    ...
    def forward(self, x_start, t, ret_pred_x=False, velocity_field=None, cond=None):
        ...
        ddpm_loss, unreduced_loss = super().forward(
            x_start, t, ret_pred_x=False, velocity_field=velocity_field, cond=cond
        )
        ...
        if self.enable_flow_matching:
            ...
            # 仅使用DDPM
            if ret_pred_x:
                velocity_field = velocity_field if velocity_field is not None else torch.randn_like(x_start)
