...
def generate_blender_screenshot_from_meshs(meshs_root_path: Path, temp_file_path: Path, output_path: Path, scale, translation, gpu_id):
...
def generate_obj_pics_blender_batched_frame(_parts_data, percentage, workspace_path: Path, scale, translation, gpu_id):
    ...
    scale, translation = generate_blender_screenshot_from_meshs(mesh_path, temp_path, rest_path, scale, translation, gpu_id)
    ...
    return scale, translation, rest_path
...
def generate_obj_pics_blender_batched(_parts_data, percentages, workspace_path: Path):
    ...
    scale, translation, _ = generate_obj_pics_blender_batched_frame(_parts_data, 1, init_path, "''", "''", 0)
    ...
    with ctx.Pool(num_workers) as pool:
        ...
        for i, percentage in enumerate(percentages):
            ...
            task_gpu_id = i % num_workers
            rt = pool.apply_async(generate_obj_pics_blender_batched_frame, (_parts_data, percentage, temp_path, scale, translation, task_gpu_id))
            handler.append(rt)
