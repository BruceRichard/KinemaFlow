...
from models.geometry_flow.dataset import DiffusionDataset
...
if __name__ == '__main__':
    ...
    if config.get('load_from_pretrained_ckpt') is not None:
        ...
        model = Flow.load_from_checkpoint(config['load_from_pretrained_ckpt'])
        Log.info('Done')
    else:
        model = Flow(config)
    ...
    checkpoint_callback = ModelCheckpoint(
            ...
            filename="flow-{epoch:04d}-{loss:.5f}",
        )
